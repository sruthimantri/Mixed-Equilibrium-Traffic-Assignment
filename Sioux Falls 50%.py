...
def k_shortest_path(linkSet):
    ...
    for s,d in list(routeSet.keys()):
        paths = list(islice(nx.shortest_simple_paths(G, s, d, weight = 'weight'), 20))#k= 10
...
def assignment(type, algorithm, accuracy):
    ...
    while gap > accuracy:
        ...
        selected_od_pairs = {x for x in diff_SO_FW if diff_SO_FW[x]>=5}
...
output = assignment("UE","FW", 0.0001)
...
import csv
with open('fw-time_50.csv', 'w') as f:
    for key in time_FW.keys():
        f.write("%s,%s\n"%(key,time_FW[key]))
        
with open('so-time_50.csv', 'w') as f:
